...
def Hamiltonian(n,t,y):
	...
	'''
	cir1 = A1(n,x,t).to_gate()
	cir2 = A2(n,y,t).to_gate()
	#cir3 = A3(n,y,t).to_gate()

	control = list(range(0, n))
	circ.append(cir1,control)
	circ.append(cir2,control)
	#circ.append(cir3,q)
	'''
...
def qft(circ,c,cn):
	...
	for i in np.arange(0,cn,1):
		for j in np.arange(0,i,1):
			circ.cp(-pi/float(2**(i-j)),c[j],c[i])
		circ.h(c[i])
...
def HHL(power,y,B,cn,shots):
	...
	c = q[an:an+cn]
	b = q[an+cn:an+cn+bn]
	t = 1/(2**cn)*2*pi/eigenvalues_sort[0]
	...
	#plot_histogram(counts).savefig('out0.png')
...
def IHHL(power, scale, y, b, cn, shots, times, learning_rate):
	A = TridiagonalToeplitz(power, 1, y)
...
[x_true, x, x_error] = IHHL(power, scale, y, b, cn, shots, times, learning_rate)
